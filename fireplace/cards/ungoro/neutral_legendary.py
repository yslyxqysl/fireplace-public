from ..utils import *
from hearthstone.enums import CardSet

##
# Minions

class UNG_840:
	"Hemet, Jungle Hunter"
	play = Discard(FRIENDLY_DECK + (COST <= 3))

#class UNG_843:
#	"Voraxx"

class UNG_851:
	"Elise the Trailblazer"
	play = Shuffle(CONTROLLER, "UNG_851t1")

class UNG_851t1:
	"Un'Goro Pack"
	play = Give(CONTROLLER, RandomCollectible(card_set=CardSet.UNGORO)) * 5

class UNG_900:
	"Spiritsinger Umbra"
	events = Summon(CONTROLLER, MINION + DEATHRATTLE).after(Deathrattle(Summon.CARD))

class UNG_907:
	"Ozruk"
	powered_up = ELEMENTAL_PLAYED_LAST_TURN
	play = powered_up & Buff(SELF, "UNG_907e") * NUM_ELEMENTAL_PLAYED_LAST_TURN(CONTROLLER)

UNG_907e = buff(health=5)
